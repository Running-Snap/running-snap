...
def run_bestcut_task(self, job_id: int, video_paths: list, photo_count: int, poster_config: dict = None, user_id: int = 0, bib: str = "", mode: str = "both"):
    db = SessionLocal()
    tmp_paths = []
    try:
        ...
        # 2순위: OpenCV
        if mode in ("original", "both") and not photos and primary_path and os.path.exists(primary_path):
            photos = extract_frames(primary_path, photo_count, OUTPUT_PHOTOS_BESTCUT_FOLDER, user_id=user_id, bib=bib)

        # 3순위: 추가 영상에서 보충
        if mode in ("original", "both") and len(photos) < photo_count and len(local_video_paths) > 1:
        ...
        # 포스터 생성
        if mode in ("poster", "both") and photos and VIDEO_EDITOR_AVAILABLE:
            try:
                import sys
        ...
        result_data = {
            "bestcut": photos if mode in ("original", "both") else [],
            "poster":  posters if mode in ("poster", "both") else [],
        }
