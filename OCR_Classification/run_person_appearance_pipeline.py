...
log = logging.getLogger("run_person_appearance_pipeline")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Pipeline modes: file(video upload) or live(camera/RTSP) "
            "with detect+track+OCR and full-frame clip outputs"
        )
    )
    p.add_argument("--mode", choices=("file", "live"), default="file")
    p.add_argument("--video", default="", help="입력 영상 경로(file 모드)")
    p.add_argument("--source", default="0", help="라이브 입력 소스(live 모드): 0, 1, rtsp://...")
    p.add_argument("--out", default="appearance_report_output", help="출력 폴더")
    p.add_argument("--ocr-backend", choices=("easyocr", "paddleocr", "noop"), default="easyocr")
    p.add_argument("--ocr-interval", type=int, default=5)
    p.add_argument("--label-policy", choices=("confidence", "frequency"), default="confidence")
    p.add_argument("--progress-log-interval", type=int, default=60)
...
def main() -> int:
    ...
    log.info("pipeline start | mode=%s", mode)
    result = run_report(cfg)
    log.info(
        "pipeline done | events=%s | clips=%s",
        result["events_count"],
        result["label_clips_count"],
    )
    log.info("track_events: %s", result["outputs"]["events_json"])
