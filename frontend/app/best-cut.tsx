...
export default function BestCutScreen() {
  ...
  const handleExtract = async () => {
    if (selectedVideos.length === 0) {
      Alert.alert('알림', '영상을 최소 1개 이상 선택해주세요.');
      return;
    }
    ...
    try {
      ...
      // 베스트 컷 작업 생성
      setLoadingText('베스트 컷 추출 중...');
      const job = await apiCreateBestcutJob(videoIds, photoCount);

      // 완료까지 폴링
      const doneJob = await pollUntilDone(() => apiGetBestcutJob(job.id));
      ...
      const msg = e instanceof Error ? e.message : '베스트 컷 추출 실패';
      Alert.alert('오류', msg);
    } finally {
      setIsLoading(false);
      setLoadingText('');
  ...
  if (isLoading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingTitle}>베스트 컷을 추출하고 있어요</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>베스트 컷 추출</Text>
      </View>

      <View style={styles.infoCard}>
        <Text style={styles.infoTitle}>📸 베스트 컷을 뽑아드려요</Text>
      ...
      <View style={styles.countSection}>
        <Text style={styles.countLabel}>추출할 사진 수: {photoCount}장</Text>
        <View style={styles.countButtons}>
          {[3, 5, 7, 10].map(n => (
            <TouchableOpacity
              key={n}
              style={[styles.countChip, photoCount === n && styles.countChipActive]}
              onPress={() => setPhotoCount(n)}
            >
              <Text style={[styles.countChipText, photoCount === n && styles.countChipTextActive]}>
                {n}장
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[styles.extractButton, selectedVideos.length === 0 && styles.extractButtonDisabled]}
          onPress={handleExtract}
          disabled={selectedVideos.length === 0}
        >
          <Text style={styles.extractButtonText}>
            베스트 컷 추출하기 ({selectedVideos.length}개 선택)
          </Text>
        </TouchableOpacity>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  countSection: { paddingHorizontal: 16, marginBottom: 8 },
  ...
  countChipActive: { backgroundColor: '#FF9500' },
  countChipText: { fontSize: 13, color: '#666' },
  countChipTextActive: { color: '#fff', fontWeight: 'bold' },
  selectionInfo: {
  ...
  addButton: {
    ...
    borderWidth: 2, borderColor: '#FF9500', borderStyle: 'dashed',
  },
  addButtonIcon: { fontSize: 24, color: '#FF9500', marginRight: 12, fontWeight: 'bold' },
  addButtonText: { fontSize: 16, color: '#FF9500', fontWeight: '600' },
  ...
  extractButton: { backgroundColor: '#FF9500', borderRadius: 12, padding: 16, alignItems: 'center' },
  extractButtonDisabled: { backgroundColor: '#ccc' },
